...
- First-dollar triage card fallback URL: `https://doctors-lighting-facial-admissions.trycloudflare.com/agent-revenue-copilot/bridge-out?offer=first_dollar_triage&action=primary_browser_checkout&source=product-readme` is the card fallback: it opens a first-party order-start page, prepares a concrete purchase id, then opens the concrete MCPlug product page before card/Stripe checkout.
